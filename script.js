...
document.addEventListener("DOMContentLoaded", () => {
  ...
  $("tokenInput").oninput = (e) =>
    localStorage.setItem("token", (state.token = e.target.value.trim()));
...
async function search(usr) {
  ...
  try {
    ...
    const resU = await fetch(`https://api.github.com/users/${usr}`, {
      headers,
    });

    if (!resU.ok) {
      throw new Error(
        resU.status === 404 ? "User not found" : "API error"
      );
    }
    ...
    const resR = await fetch(
      `https://api.github.com/users/${usr}/repos?per_page=100`,
      { headers }
    );
...
function display(user, repos) {
  ...
  $("metaLoc").textContent = user.location
    ? `📍 ${user.location}`
    : "";
  ...
  $("metaJoined").textContent = `📅 Joined ${new Date(
    user.created_at
  ...
  const badges = [
    ...
    new Set(repos.map((r) => r.language).filter(Boolean)).size >= 4 &&
      "🚀 Polyglot",
    (Date.now() - new Date(user.created_at)) / 31557600000 >= 8 &&
      "🎓 Veteran",
  ].filter(Boolean);

  $("achievementsList").innerHTML = (
    badges.length ? badges : ["🌱 Rising Star"]
  )
    .map((b) => `<span class="badge-tag">${b}</span>`)
    .join("");
        ...
        .map((l) => {
          ...
          return `
            <div
              ...
              style="width:${l.pct}%; background:${col}"
  ...
  $("langLegend").innerHTML = total
    ? sorted
        ...
        .join("")
    : "<span>No language metrics found.</span>";
...
function renderRepos() {
  ...
  $("reposList").innerHTML = list.length
    ? list
        .slice(0, 30)
        .map(
          (r) => `
